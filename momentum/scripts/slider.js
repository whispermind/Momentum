...
export default async function slider(update) {
  ...
  if (settings.api === 'unsplash') unsplash = await getData(`https://api.unsplash.com/search/photos/?client_id=SaIx04gu2ik9Jsm_ExeT4Ah7u5sXqhPnd7bo-oZouyA&orientation=landscape&query=${settings.tags ? settings.tags : await getDayTime()}&per_page=20`);
  if (settings.api === 'flickr') flickr = await getData(`https://www.flickr.com/services/rest/?method=flickr.photos.search&api_key=f1d1e25e63b77ed91af6b5fe0b0a65ac&tags=${settings.tags ? settings.tags : await getDayTime()}&extras=url_l&format=json&nojsoncallback=1&per_page=20`);
  let processed = false;
  ...
  async function setImage() {
    ...
    if (settings.api === 'git') url = `https://raw.githubusercontent.com/whispermind/stage1-tasks/assets/images/${await getDayTime()}/${SLIDER_NAV.dataset.current < 9 ? '0' + (Number(SLIDER_NAV.dataset.current) + 1) : Number(SLIDER_NAV.dataset.current) + 1}.jpg`;
