export default async function weather(update) {
  ...
  STORAGE ? (CITY.value = STORAGE) : (STORAGE = "Kyiv");
  setWeather(STORAGE);
  ...
  async function setWeather(city) {
    ...
    if (info.cod !== 200) {
      ERROR.textContent = settings.lang === "english" ? "City not found" : "Город не найден"
      clearWeather();
      return;
    }
    ERROR.textContent = "";
    ...
    HUMIDITY.textContent = `${
      settings.lang === "english" ? "Humidity" : "Влажность воздуха"
    }: ${Math.round(info.main.humidity)}%`;
    WIND.textContent = `${
      settings.lang === "english" ? "Wind speed" : "Скорость ветра"
    }: ${Math.round(info.wind.speed)} ${
      settings.lang === "english" ? "m/s" : "м/с"
    }`;
  ...
  async function getWeather(city) {
    ...
    const response = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=66402a9e47202c888a359d9eb072bd6d&units=metric&lang=${
        translation[settings.lang].weather
      }`
    );
