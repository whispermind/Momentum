...
export default function player(update) {
  ...
  if (!update) {
    ...
    RANGE.addEventListener("change", (event) => {
      ...
      RANGE.dataset.current = `${Math.floor(AUDIO.currentTime)}s / ${
  ...
  function setSong() {
    ...
    if (active) {
      ...
      active.querySelector("button").classList.remove("pause");
    }
    LIST.querySelector(`li:nth-child(${current + 1})`).classList.add(
      "item-active"
    );

    if (PLAY.classList.contains("pause")) {
      LIST.querySelector(`li:nth-child(${current + 1}) button`).classList.add(
        "pause"
      );
      AUDIO.play();
    }
